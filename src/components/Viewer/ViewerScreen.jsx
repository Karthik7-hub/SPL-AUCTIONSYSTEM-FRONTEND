...
// Add 'Sparkles', 'Zap', 'Mic2' to your imports
import { Trophy, Users, List, Gavel, DollarSign, TrendingUp, Shield, CheckCircle, AlertCircle, Clock, Filter, Grid, Sparkles, Zap, Mic2 } from 'lucide-react';
export default function ViewerScreen({ data, liveState }) {
    const [activeTab, setActiveTab] = useState('live');
    const [viewStatus, setViewStatus] = useState('OPEN'); // 'OPEN', 'SOLD', 'UNSOLD', 'ALL'
        ...
        <div className="min-h-screen bg-slate-900 text-slate-100 font-sans flex flex-col pb-24 md:pb-0">
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8">
                <div className="max-w-7xl mx-auto">
                    ...
                    {/* === PLAYER POOL TAB (REDESIGNED FILTERS) === */}
                    {activeTab === 'players' && (
                        <div className="space-y-6">

                            {/* 1. Sticky Mobile Filter Grid (No Scrolling) */}
                            <div className="sticky top-[-1rem] md:static z-40 bg-slate-900/95 backdrop-blur-md py-3 -mx-4 px-4 border-b border-slate-800 md:border-none md:p-0 md:mx-0 md:bg-transparent">
                                <div className="grid grid-cols-4 gap-2 md:flex md:w-auto">
                                    ...
                                    <FilterButton
                                        label="Open"
                                        active={viewStatus === 'OPEN'}
                                        onClick={() => setViewStatus('OPEN')}
                                        colorClass="bg-blue-600 text-white"
                                    />
                                    <FilterButton
                                        label="Sold"
                                        active={viewStatus === 'SOLD'}
                                        onClick={() => setViewStatus('SOLD')}
                                        colorClass="bg-green-600 text-white"
                                    />
                                    <FilterButton
                                        label="Unsold"
                                        active={viewStatus === 'UNSOLD'}
                                        onClick={() => setViewStatus('UNSOLD')}
                                        colorClass="bg-red-600 text-white"
                                    />
                                    <FilterButton
                                        label="All"
                                        active={viewStatus === 'ALL'}
                            ...
                            {/* 2. Desktop Category Filter (Hidden on Mobile) */}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                                {safePlayers
                                    ...
                                    .map(p => {
                                        ...
                                        return (
                                            <div key={p._id} className={`relative p-5 rounded-2xl border transition-all ${p.isSold ? 'bg-slate-800 border-slate-700' :
                                                p.isUnsold ? 'bg-red-900/5 border-red-900/20' :
                                                    'bg-slate-800 border-slate-700'
                                                }`}>

                                                {/* Sold Badge */}
                                                {p.isSold && (
                                                    <div className="absolute top-4 right-4 text-green-500"><CheckCircle className="w-6 h-6" /></div>
                                                )}
                                                ...
                                                <div className="pt-4 border-t border-slate-700/50 flex justify-between items-end">
                                                    <div>
                                                        {p.isSold ? (
                                                            <div className="flex flex-col">
                                                                ...
                                                                <span className="text-sm font-bold" style={{ color: soldToTeam?.color || '#fff' }}>
                                                                    {soldToTeam?.name || 'Unknown'}
                                                                </span>
                                                            </div>
                                                        ) : p.isUnsold ? (
                                                    ...
                                                    <div className="text-right">
                                                        ...
                                                        <div className={`text-lg font-mono font-bold ${p.isSold ? 'text-green-400' : 'text-yellow-500'}`}>
                            ...
                            {safePlayers.filter(p => {
                                ...
                                if (viewStatus === 'UNSOLD') return p.isUnsold;
                                return true;
                            }).filter(p => selectedCategory === 'All' || p.category === selectedCategory).length === 0 && (
                                    <div className="flex flex-col items-center justify-center py-20 text-slate-500 border-2 border-dashed border-slate-800 rounded-2xl bg-slate-800/20">
                    ...
                    {activeTab === 'sold' && (
                        <div className="bg-slate-800 rounded-2xl overflow-hidden shadow-xl border border-slate-700">
                            <div className="p-4 bg-slate-900/50 border-b border-slate-700 text-xs font-bold text-slate-500 uppercase tracking-wider">
                                Live Transfer Feed
                            </div>
                            <div className="divide-y divide-slate-700">
                                {[...safePlayers].reverse().filter(p => p.isSold).map(p => {
                                    ...
                                    return (
                                        <div key={p._id} className="p-4 flex items-center justify-between hover:bg-slate-700/30 transition-colors">
                                            ...
                                            <div className="text-right">
                                                <div className="text-xs font-bold mb-1 px-2 py-0.5 rounded text-white inline-block" style={{ backgroundColor: team?.color || '#555' }}>
                                                    {team?.name || 'Unknown'}
                                                </div>
                                ...
                                {safePlayers.filter(p => p.isSold).length === 0 && (
                                    <div className="p-12 text-center text-slate-500 italic">No sold players yet.</div>
                                )}
                            </div>
                        </div>
                    )}
...
function FilterButton({ label, active, onClick, colorClass }) {
    return (
        <button
            onClick={onClick}
            className={`rounded-lg py-2.5 text-xs font-bold uppercase tracking-wider transition-all transform active:scale-95 ${active
                ? `${colorClass} shadow-lg ring-1 ring-white/20`
...
// === IMPROVED LIVE AUCTION COMPONENT ===
function LiveAuctionView({ data, liveState }) {
    ...
    if ((liveState?.status === 'SOLD' || liveState?.status === 'UNSOLD') && currentPlayer) {
        ...
        return (
            <div className="relative flex flex-col items-center justify-center min-h-[70vh] w-full overflow-hidden rounded-3xl bg-slate-900 border border-slate-700 shadow-2xl">
                ...
                {/* Confetti / Particle Effect (CSS only) */}
                {isSold && (
                    <div className="absolute inset-0 overflow-hidden pointer-events-none">
                        <div className="absolute top-1/4 left-1/4 w-2 h-2 bg-yellow-400 rounded-full animate-ping"></div>
                        <div className="absolute top-1/3 right-1/4 w-3 h-3 bg-blue-400 rounded-full animate-bounce"></div>
                        <div className="absolute bottom-1/4 left-1/2 w-2 h-2 bg-red-400 rounded-full animate-ping"></div>
                    </div>
                )}
                ...
                <div className="relative z-10 flex flex-col items-center text-center p-8 animate-in zoom-in duration-500">
                    ...
                    <div className="relative mb-10">
                        <div className={`text-[4rem] md:text-[8rem] font-black leading-none transform -rotate-6 border-8 px-8 py-2 rounded-xl border-double shadow-2xl backdrop-blur-sm
               ${isSold
                                ? 'text-yellow-400 border-yellow-500 bg-yellow-900/20 shadow-yellow-500/20'
                    ...
                    {isSold ? (
                        <div className="bg-slate-800/80 backdrop-blur-md p-6 md:p-10 rounded-3xl border border-slate-600 shadow-2xl flex flex-col gap-4 min-w-[300px]">
                            <div>
                                ...
                                <div className="text-2xl md:text-4xl font-black text-white px-4 py-2 rounded-lg shadow-inner" style={{ backgroundColor: leadingTeam?.color }}>
                                    {leadingTeam?.name}
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="text-4xl md:text-6xl font-mono font-bold text-green-400 drop-shadow-lg">
                                    ₹{liveState.currentBid}L
                                </div>
                            </div>
                        </div>
                    ) : (
                        <div className="text-slate-500 text-xl font-medium bg-slate-800/50 px-8 py-4 rounded-full border border-slate-700">
                            Pass to next round
                        </div>
                    )}
    ...
    if (liveState?.status === 'ACTIVE' && currentPlayer) {
        return (
            <div className="flex flex-col lg:flex-row gap-4 h-full min-h-[70vh]">
                ...
                {/* LEFT: PLAYER CARD (Trading Card Style) */}
                <div className="w-full lg:w-5/12 relative group perspective">
                    ...
                    <div className="relative h-full bg-slate-800 rounded-[2rem] overflow-hidden border border-slate-600 shadow-2xl flex flex-col">
                        ...
                        <div className="absolute -right-10 -top-10 text-slate-700/20">
                        ...
                        <div className="relative z-10 p-8 flex flex-col h-full justify-between">
                            <div>
                                <div className="flex gap-2 mb-4">
                                    <span className="px-3 py-1 bg-blue-600 text-white rounded-lg text-xs font-bold uppercase tracking-wider shadow-lg shadow-blue-500/30">
                                        {currentPlayer.category}
                                    </span>
                                </div>

                                <h1 className="text-5xl md:text-7xl font-black text-white leading-[0.9] mb-4 drop-shadow-xl tracking-tight">
                                    {currentPlayer.name.split(' ').map((word, i) => (
                                        <span key={i} className="block">{word}</span>
                                    ))}
                                </h1>

                                <div className="text-2xl font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                ...
                {/* RIGHT: BIDDING ENGINE (Scoreboard Style) */}
                <div className="w-full lg:w-7/12 flex flex-col gap-4">

                    {/* Scoreboard */}
                    <div className={`relative flex-1 rounded-[2rem] p-8 md:p-12 flex flex-col items-center justify-center shadow-2xl border transition-colors duration-500 overflow-hidden
            ${leadingTeam
                    ...
                    >
                        ...
                        {leadingTeam && (
                            <div
                                ...
                                style={{ background: `radial-gradient(circle at center, ${leadingTeam.color}, transparent 70%)` }}
                            ></div>
                        )}
                        ...
                        <div className="relative z-10 text-[6rem] md:text-[10rem] font-black font-mono leading-none tracking-tighter text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.3)] mb-8">
                            <span className="text-4xl md:text-6xl text-slate-500 mr-2">₹</span>
                            {liveState.currentBid}
                            <span className="text-2xl md:text-4xl text-slate-500 ml-2">L</span>
                        </div>
                            ...
                            {leadingTeam ? (
                                <div className="bg-white/10 backdrop-blur-md border border-white/20 p-4 rounded-2xl flex items-center justify-between animate-in slide-in-from-bottom-4 duration-300">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 rounded-xl shadow-lg flex items-center justify-center text-white font-bold text-xl" style={{ backgroundColor: leadingTeam.color }}>
                                ...
                                <div className="text-center p-6 border-2 border-dashed border-slate-700 rounded-2xl text-slate-500 font-bold uppercase tracking-wider animate-pulse">
    ...
    return (
        <div className="flex flex-col items-center justify-center h-[60vh] relative overflow-hidden rounded-3xl bg-slate-900 border border-slate-800">
            ...
            <div className="relative z-10 bg-slate-800 p-8 rounded-full shadow-2xl border border-slate-700 mb-8">
                <Mic2 className="w-16 h-16 text-blue-500 animate-pulse" />
            </div>

            <h2 className="relative z-10 text-3xl md:text-5xl font-black text-white tracking-tight mb-2">
                Auction Floor
            </h2>
